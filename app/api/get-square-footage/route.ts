...
export async function POST(request: NextRequest) {
  try {
    ...
    const serpApiKey = settings.serpapi_api_key as string | undefined

    // --- NEW LOGGING ---
    console.log(`SerpAPI Key being used: ${serpApiKey ? "Configured" : "NOT CONFIGURED"}`)
    if (!serpApiKey) {
      console.error("SerpAPI key is not configured in settings. Please ensure it's saved in the dashboard.")
      return NextResponse.json(
        {
          success: false,
          error: "SerpAPI key is not configured. Please set it in the dashboard settings.",
    ...
    for (const query of searchQueries) {
      try {
        ...
        console.log(`SerpAPI Request URL: ${searchUrl}`) // Log the full URL
        ...
        // --- NEW LOGGING ---
        console.log(`Raw SerpAPI Search Results for "${query}":`, JSON.stringify(searchResults, null, 2))
        if (searchResults.error) {
          console.error("SerpAPI returned an error in its response:", searchResults.error)
        }
        // --- END NEW LOGGING ---
    ...
    if (!propertyData || !propertyData.squareFootage) {
      console.log("No property data or square footage found after all search attempts")
      return NextResponse.json(
        {
          success: false,
